...
func Test_ReqRep1(t *testing.T) {
	...
	n1.Call(56, []byte("Hellow"), 57, testError)
...
func Test_PubSub1(t *testing.T) {
	...
	n1.Publish(channel, []byte("Broadcast"), testError)
	time.Sleep(50 * time.Millisecond)
	ctxExitFunc()
	time.Sleep(50 * time.Millisecond)
...
func Test_PubSub2(t *testing.T) {
	...
	n2.Subscribe(channel, testReply)
	time.Sleep(1000 * time.Millisecond)
	n1.Publish(channel, []byte("Broadcast1"), testError)
	time.Sleep(50 * time.Millisecond)
	n2.Unsubscribe(channel)
	time.Sleep(1500 * time.Millisecond)
	n1.Publish(channel, []byte("Broadcast2"), testError)
	time.Sleep(50 * time.Millisecond)
	ctxExitFunc()
	time.Sleep(50 * time.Millisecond)
	testTableVerificationDisorder(t, []string{
...
func Test_Bus1(t *testing.T) {
	...
	time.Sleep(1000 * time.Millisecond)
	n1.Publish(bus, []byte("Broadcast1"), testError)
	n2.Publish(bus, []byte("Broadcast2"), testError)
...
func Test_Bus2(t *testing.T) {
	...
	n4.Subscribe(bus, testReply)
	time.Sleep(1000 * time.Millisecond)
	n1.Publish(bus, []byte("speech1"), testError)
	time.Sleep(50 * time.Millisecond)
	n1.Unsubscribe(bus)
	time.Sleep(1000 * time.Millisecond)
	n3.Publish(bus, []byte("speech2"), testError)
	time.Sleep(50 * time.Millisecond)
	n3.Unsubscribe(bus)
	time.Sleep(1000 * time.Millisecond)
	n2.Publish(bus, []byte("speech3"), testError)
	time.Sleep(50 * time.Millisecond)
	n4.Unsubscribe(bus)
	time.Sleep(1000 * time.Millisecond)
	ctxExitFunc()
...
func Test_WatchChannel(t *testing.T) {
	...
	time.Sleep(500 * time.Millisecond)
	n1.Call(90, []byte("Hellow"), 0, testError)
	data := <-cc
